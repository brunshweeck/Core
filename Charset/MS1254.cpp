...
gchar MS1254::decode(gint b) {
    return bytesToChar[b + 128];
}

Charset::CoderResult MS1254::decodeLoop(ByteBuffer &src, CharBuffer &dst) {
...
Charset::CoderResult MS1254::encodeLoop(CharBuffer &src, ByteBuffer &dst) {
    gint mark = src.position();
    try {
        while (src.hasRemaining()) {
            gchar c = src.get();
...
gfloat MS1254::averageCharsPerByte() const {
    return 1;
}

gfloat MS1254::averageBytesPerChar() const {
    return 1;
}

gbool MS1254::contains(const Charset &cs) const {
    return dynamic_cast<MS1254 const *>(&cs) || name().equals("US-ASCII");
}

Object &MS1254::clone() const {
    static MS1254 ms1254 = MS1254();
    return ms1254;
}

gint MS1254::encode(gchar ch) {
