...
MS950::MS950() : Charset("Windows-1251") {}

String MS950::name() const {
    return "Windows-1251";
}

gchar MS950::decode(gint b) {
    return bytesToChar[b + 128];
}

Charset::CoderResult MS950::decodeLoop(ByteBuffer &src, CharBuffer &dst) {
...
Charset::CoderResult MS950::encodeLoop(CharBuffer &src, ByteBuffer &dst) {
    gint mark = src.position();
    try {
        while (src.hasRemaining()) {
            gchar c = src.get();
...
gfloat MS950::averageCharsPerByte() const {
    return 1;
}

gfloat MS950::averageBytesPerChar() const {
    return 1;
}

gbool MS950::contains(const Charset &cs) const {
    return dynamic_cast<MS950 const *>(&cs) || name().equals("US-ASCII");
}

Object &MS950::clone() const {
    static MS950 ms950 = MS950();
    return ms950;
}

gint MS950::encode(gchar ch) {
