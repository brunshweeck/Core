...
MS949::MS949() : Charset("Windows-1251") {}

String MS949::name() const {
    return "Windows-1251";
}

gchar MS949::decode(gint b) {
    return bytesToChar[b + 128];
}

Charset::CoderResult MS949::decodeLoop(ByteBuffer &src, CharBuffer &dst) {
...
Charset::CoderResult MS949::encodeLoop(CharBuffer &src, ByteBuffer &dst) {
    gint mark = src.position();
    try {
        while (src.hasRemaining()) {
            gchar c = src.get();
...
gfloat MS949::averageCharsPerByte() const {
    return 1;
}

gfloat MS949::averageBytesPerChar() const {
    return 1;
}

gbool MS949::contains(const Charset &cs) const {
    return dynamic_cast<MS949 const *>(&cs) || name().equals("US-ASCII");
}

Object &MS949::clone() const {
    static MS949 ms949 = MS949();
    return ms949;
}

gint MS949::encode(gchar ch) {
