...
gchar MS1252::decode(gint b) {
    return bytesToChar[b + 128];
}

Charset::CoderResult MS1252::decodeLoop(ByteBuffer &src, CharBuffer &dst) {
...
Charset::CoderResult MS1252::encodeLoop(CharBuffer &src, ByteBuffer &dst) {
    gint mark = src.position();
    try {
        while (src.hasRemaining()) {
            gchar c = src.get();
...
gfloat MS1252::averageCharsPerByte() const {
    return 1;
}

gfloat MS1252::averageBytesPerChar() const {
    return 1;
}

gbool MS1252::contains(const Charset &cs) const {
    return dynamic_cast<MS1252 const *>(&cs) || name().equals("US-ASCII");
}
