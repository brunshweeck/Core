...
MS1255::MS1255() : Charset("Windows-1255") {}

MS1255 MS1255::INSTANCE{};

String MS1255::name() const {
    return "Windows-1255";
}

gchar MS1255::decode(gint b) {
    return bytesToChar[b + 128];
}

Charset::CoderResult MS1255::decodeLoop(ByteBuffer &src, CharBuffer &dst) {
...
Charset::CoderResult MS1255::encodeLoop(CharBuffer &src, ByteBuffer &dst) {
    gint mark = src.position();
    try {
        while (src.hasRemaining()) {
            gchar c = src.get();
...
gfloat MS1255::averageCharsPerByte() const {
    return 1;
}

gfloat MS1255::averageBytesPerChar() const {
    return 1;
}

gbool MS1255::contains(const Charset &cs) const {
    return dynamic_cast<MS1255 const *>(&cs) || name().equals("US-ASCII");
}

Object &MS1255::clone() const {
    static MS1255 ms1255 = MS1255();
    return ms1255;
}

gint MS1255::encode(gchar ch) {
...
Charset::ErrorAction MS1255::malformedAction() const {
    return Charset::malformedAction();
}

Charset::ErrorAction MS1255::unmappableAction() const {
    return Charset::unmappableAction();
}

CharBuffer MS1255::decode(ByteBuffer &in) {
    return Charset::decode(in);
}

ByteBuffer MS1255::encode(CharBuffer &in) {
    return Charset::encode(in);
}

String MS1255::toString() const {
